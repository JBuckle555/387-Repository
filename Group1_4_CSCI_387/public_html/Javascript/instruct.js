...
function searchStudent(studentID,classID) {
  if (classID!==undefined){

      var query = "?student="+ studentID +"&classID="+classID;
  }else{
  
    var query = "?student="+ studentID +"&classID="+"";
  }
  ...
  function searchCourse() {
    ...
    if (index <0){
      var department = string.substr(index + 1);
      var courseID ="";
    }else{
...
function displayStudent(classID) {
  ...
  //////////Register for class from Student //////////////////////////////
  function register(classID) {
    var courseID_HTML = "courseID"+classID;
    console.log(classID);
    console.log(courseID_HTML);
    var string = document.getElementById(courseID_HTML).innerText;
    var index = string.indexOf(" ");
    var department = string.substr(0, index);
    var courseID = string.substr(index + 1);

    var query = "?department="+ department +"&courseID="+courseID+"&classID="+classID;
          location.href = "addCourse.php"+query;
      }
/////////// Force Delete Student from a Course//////////////////////////
function deleteStudent(StudentID){
  if(confirm("Do you want to delete this person from this course?")){
